...
export default async function handler(req, res) {
  ...
  const { course_name, student_name, email, phone, notes, price } = req.body || {};
  ...
  try {
    ...
    const mailOptions = {
      ...
      subject: `[Careax-수강신청] ${student_name}님이 '${course_name}'를 신청했습니다. (예상매출 +${computedPrice.toLocaleString()}원)`,
      ...
      html: `
        <div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#1a1a1a;border:1px solid #eee;padding:24px;border-radius:8px;">
          ...
          <p style="font-size:15px;line-height:1.6;margin-bottom:1.5rem;text-align:center;font-weight:600;color:#111;">
            "${course_name}" 강의 신청해 주셔서 감사합니다.<br>
            확인하고 메일로 다시 연락드리겠습니다.
          </p>
          <table cellpadding="8" style="width:100%;border-collapse:collapse;background:#f9f9f9;border-radius:6px;overflow:hidden;margin-top:1.5rem;">
            ...
            <tr><td style="font-weight:600;border-bottom:1px solid #eee;">결제금액</td><td style="border-bottom:1px solid #eee;color:#A855F7;font-weight:600;">${computedPrice.toLocaleString()}원 (테스트 결제)</td></tr>
